...
class DatabasesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create database" do
    assert_difference('Database.count') do
      post databases_url, params: { database: { name: @database.name, notes: @database.notes } }
    end
  ...
  test "should update database" do
    patch database_url(@database), params: { database: { name: @database.name, notes: @database.notes } }
